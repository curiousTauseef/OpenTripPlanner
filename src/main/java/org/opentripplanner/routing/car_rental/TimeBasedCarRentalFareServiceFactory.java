...
public class TimeBasedCarRentalFareServiceFactory implements FareServiceFactory {
    ...
    private static Logger log = LoggerFactory.getLogger(TimeBasedCarRentalFareServiceFactory.class);
    ...
    public FareService makeFareService() {
        return new TimeBasedCarRentalFareService(currency, pricingBySecond);
    }
    ...
    public void configure(JsonNode config) {
        ...
        for (P2<Integer> bracket : pricingBySecond) {
            ...
            if (maxTime == seconds) {
                throw new IllegalArgumentException("Car share pricing has two entries for "
                        + maxTime);
            }
            if (cost < lastCost) {
                log.warn("Car share pricing has pathological pricing; keeping the car for a "
